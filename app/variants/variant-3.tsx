"use client";

import {Playfair_Display, Sora} from 'next/font/google';
import { motion } from 'framer-motion';
import { Shield, TrendingUp, Users, Zap, CheckCircle, Star, ArrowRight, CreditCard, BarChart3, Globe, Lock, Headphones } from 'lucide-react';

const playfair = Playfair_Display({ subsets: ['latin'] });
const sora = Sora({ subsets: ['latin'] });

export default function WhopLanding() {
  return (
    <div className={`min-h-screen bg-gradient-to-br from-slate-50 via-white to-amber-50/30 ${sora.className}`}>
      {/* Navigation */}
      <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-md border-b border-slate-200/50 z-50" data-landright-section="Navigation">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              <img src="data:image/webp;base64," alt="Whop" className="h-8 w-auto" />
              <span className="text-xl font-semibold text-slate-900">Whop</span>
            </div>
            <div className="hidden md:flex items-center space-x-8">
              <span className="text-slate-600 hover:text-slate-900 cursor-pointer transition-colors">Platform</span>
              <span className="text-slate-600 hover:text-slate-900 cursor-pointer transition-colors">Solutions</span>
              <span className="text-slate-600 hover:text-slate-900 cursor-pointer transition-colors">Resources</span>
              <a href="https://www.linkedin.com/company/whop-io/about/" className="bg-gradient-to-r from-violet-600 to-purple-600 text-white px-6 py-2 rounded-lg font-medium hover:from-violet-700 hover:to-purple-700 transition-all duration-200 shadow-lg hover:shadow-xl">
                Get started
              </a>
            </div>
          </div>
        </div>
      </nav>

      {/* Hero Section */}
      <section className="pt-24 pb-20 px-4 sm:px-6 lg:px-8" data-landright-section="Hero">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8 }}
              className="relative"
            >
              <div className="absolute inset-0 bg-gradient-to-r from-violet-600/20 to-amber-400/20 blur-3xl rounded-full transform -translate-y-12"></div>
              <h1 className={`${playfair.className} text-5xl md:text-7xl lg:text-8xl font-bold text-slate-900 mb-6 relative`}>
                Build Your
                <span className="block bg-gradient-to-r from-violet-600 to-amber-500 bg-clip-text text-transparent">
                  Digital Empire
                </span>
              </h1>
            </motion.div>
            
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.2 }}
              className="text-xl md:text-2xl text-slate-600 mb-12 max-w-4xl mx-auto leading-relaxed"
            >
              The premium platform powering the next generation of digital commerce. 
              Create sophisticated storefronts, process payments seamlessly, and scale your business with enterprise-grade tools.
            </motion.p>
            
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, delay: 0.4 }}
              className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16"
            >
              <a href="https://www.linkedin.com/company/whop-io/about/" className="bg-gradient-to-r from-violet-600 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg hover:from-violet-700 hover:to-purple-700 transition-all duration-200 shadow-xl hover:shadow-2xl hover:scale-105 flex items-center gap-2">
                Get started
                <ArrowRight className="w-5 h-5" />
              </a>
              <div className="flex items-center gap-2 text-slate-600">
                <Star className="w-5 h-5 text-amber-500 fill-current" />
                <span className="font-medium">Trusted by 10,000+ merchants worldwide</span>
              </div>
            </motion.div>
          </div>

          {/* Geometric Pattern Overlay */}
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 1, delay: 0.6 }}
            className="relative max-w-6xl mx-auto"
          >
            <div className="absolute inset-0 bg-gradient-to-br from-violet-100 to-amber-100 rounded-3xl transform rotate-1"></div>
            <div className="absolute inset-0 bg-gradient-to-tl from-purple-100 to-slate-100 rounded-3xl transform -rotate-1"></div>
            <div className="relative bg-white rounded-3xl shadow-2xl p-8 border border-slate-200">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center p-6 bg-gradient-to-br from-violet-50 to-purple-50 rounded-2xl border border-violet-200">
                  <div className="w-16 h-16 bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl mx-auto mb-4 flex items-center justify-center">
                    <CreditCard className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="font-semibold text-slate-900 mb-2">Payment Processing</h3>
                  <p className="text-slate-600 text-sm">Secure, instant payments with global reach</p>
                </div>
                <div className="text-center p-6 bg-gradient-to-br from-amber-50 to-yellow-50 rounded-2xl border border-amber-200">
                  <div className="w-16 h-16 bg-gradient-to-br from-amber-500 to-yellow-600 rounded-2xl mx-auto mb-4 flex items-center justify-center">
                    <BarChart3 className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="font-semibold text-slate-900 mb-2">Advanced Analytics</h3>
                  <p className="text-slate-600 text-sm">Deep insights to optimize your business</p>
                </div>
                <div className="text-center p-6 bg-gradient-to-br from-slate-50 to-gray-50 rounded-2xl border border-slate-200">
                  <div className="w-16 h-16 bg-gradient-to-br from-slate-500 to-gray-600 rounded-2xl mx-auto mb-4 flex items-center justify-center">
                    <Globe className="w-8 h-8 text-white" />
                  </div>
                  <h3 className="font-semibold text-slate-900 mb-2">Global Marketplace</h3>
                  <p className="text-slate-600 text-sm">Reach customers across every continent</p>
                </div>
              </div>
            </div>
          </motion.div>
        </div>
      </section>

      {/* Features Section */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-900 via-slate-800 to-violet-900" data-landright-section="Features">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className={`${playfair.className} text-4xl md:text-6xl font-bold text-white mb-6`}>
              Enterprise-Grade Capabilities
            </h2>
            <p className="text-xl text-slate-300 max-w-3xl mx-auto">
              Every tool you need to build, scale, and optimize your digital business with institutional-quality infrastructure.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: Shield, title: 'Storefront Builder', desc: 'Drag-and-drop interface with premium templates' },
              { icon: CreditCard, title: 'Payment Processing', desc: 'Multi-currency support with instant settlements' },
              { icon: TrendingUp, title: 'Marketplace Discovery', desc: 'AI-powered product recommendations and SEO' },
              { icon: BarChart3, title: 'Analytics Dashboard', desc: 'Real-time insights with predictive modeling' },
              { icon: Users, title: 'Customer Management', desc: 'CRM integration with automated workflows' },
              { icon: Zap, title: 'Digital Delivery', desc: 'Instant product delivery with access controls' },
              { icon: Globe, title: 'Mobile Optimization', desc: 'Native mobile experience across all devices' },
              { icon: Lock, title: 'Fraud Protection', desc: 'Machine learning security with 99.9% accuracy' }
            ].map((feature, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 hover:bg-white/15 transition-all duration-300"
              >
                <feature.icon className="w-12 h-12 text-amber-400 mb-4" />
                <h3 className="text-xl font-semibold text-white mb-3">{feature.title}</h3>
                <p className="text-slate-300">{feature.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Social Proof Section */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-violet-50 to-amber-50" data-landright-section="SocialProof">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className={`${playfair.className} text-4xl md:text-5xl font-bold text-slate-900 mb-12`}>
            Powering Digital Commerce Leaders
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            <div className="bg-white rounded-2xl p-8 shadow-xl border border-slate-200">
              <div className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-violet-600 to-purple-600 bg-clip-text text-transparent mb-2">
                $1B+
              </div>
              <p className="text-slate-600 font-medium">Total Payments Processed</p>
            </div>
            <div className="bg-white rounded-2xl p-8 shadow-xl border border-slate-200">
              <div className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-amber-500 to-yellow-600 bg-clip-text text-transparent mb-2">
                10K+
              </div>
              <p className="text-slate-600 font-medium">Active Merchants</p>
            </div>
            <div className="bg-white rounded-2xl p-8 shadow-xl border border-slate-200">
              <div className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-emerald-500 to-green-600 bg-clip-text text-transparent mb-2">
                95%
              </div>
              <p className="text-slate-600 font-medium">Seller Satisfaction Rate</p>
            </div>
          </div>

          <div className="bg-white rounded-3xl p-8 shadow-2xl border border-slate-200 max-w-4xl mx-auto">
            <div className="flex items-center justify-center mb-6">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-6 h-6 text-amber-500 fill-current" />
              ))}
            </div>
            <blockquote className={`${playfair.className} text-2xl md:text-3xl text-slate-900 mb-6 italic`}>
              "Whop transformed our digital business from a side project to a seven-figure enterprise. The platform's sophistication rivals any enterprise solution."
            </blockquote>
            <div className="flex items-center justify-center gap-4">
              <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold">
                MK
              </div>
              <div className="text-left">
                <p className="font-semibold text-slate-900">Maria Kowalski</p>
                <p className="text-slate-600">Founder, Digital Mastery Co.</p>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Process Section */}
      <section className="py-20 px-4 sm:px-6 lg:px-8" data-landright-section="Process">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className={`${playfair.className} text-4xl md:text-5xl font-bold text-slate-900 mb-6`}>
              Launch in Four Simple Steps
            </h2>
            <p className="text-xl text-slate-600 max-w-3xl mx-auto">
              From concept to revenue in minutes, not months. Our streamlined onboarding gets you selling immediately.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { step: '01', title: 'Sign Up', desc: 'Create your account with enterprise-grade security in under 60 seconds' },
              { step: '02', title: 'Create Storefront', desc: 'Choose from premium templates or build custom with our visual editor' },
              { step: '03', title: 'Upload Products', desc: 'Add digital products with automated delivery and access controls' },
              { step: '04', title: 'Start Selling', desc: 'Go live instantly with integrated payments and global distribution' }
            ].map((item, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: index * 0.2 }}
                className="relative"
              >
                <div className="bg-gradient-to-br from-white to-slate-50 rounded-2xl p-8 shadow-xl border border-slate-200 h-full">
                  <div className="text-6xl font-bold bg-gradient-to-r from-violet-600 to-amber-500 bg-clip-text text-transparent mb-4">
                    {item.step}
                  </div>
                  <h3 className="text-xl font-semibold text-slate-900 mb-4">{item.title}</h3>
                  <p className="text-slate-600">{item.desc}</p>
                </div>
                {index < 3 && (
                  <div className="hidden lg:block absolute top-1/2 -right-4 transform -translate-y-1/2">
                    <ArrowRight className="w-8 h-8 text-slate-400" />
                  </div>
                )}
              </motion.div>
            ))}
          </div>
          
          <div className="text-center mt-16">
            <a href="https://www.linkedin.com/company/whop-io/about/" className="bg-gradient-to-r from-violet-600 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold text-lg hover:from-violet-700 hover:to-purple-700 transition-all duration-200 shadow-xl hover:shadow-2xl hover:scale-105 inline-flex items-center gap-2">
              Get started
              <ArrowRight className="w-5 h-5" />
            </a>
          </div>
        </div>
      </section>

      {/* Trust & Security Section */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-50 to-violet-50" data-landright-section="Trust">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className={`${playfair.className} text-4xl md:text-5xl font-bold text-slate-900 mb-6`}>
              Enterprise Security & Compliance
            </h2>
            <p className="text-xl text-slate-600 max-w-3xl mx-auto">
              Bank-level security with global compliance standards. Your business and customers are protected by institutional-grade infrastructure.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="bg-white rounded-2xl p-8 shadow-xl border border-slate-200">
              <Shield className="w-12 h-12 text-violet-600 mb-4" />
              <h3 className="text-xl font-semibold text-slate-900 mb-4">SOC 2 Certified</h3>
              <p className="text-slate-600">Audited security controls meeting the highest industry standards for data protection and privacy.</p>
            </div>
            <div className="bg-white rounded-2xl p-8 shadow-xl border border-slate-200">
              <Lock className="w-12 h-12 text-amber-600 mb-4" />
              <h3 className="text-xl font-semibold text-slate-900 mb-4">256-bit Encryption</h3>
              <p className="text-slate-600">Military-grade encryption for all data transmission and storage with zero-knowledge architecture.</p>
            </div>
            <div className="bg-white rounded-2xl p-8 shadow-xl border border-slate-200">
              <CheckCircle className="w-12 h-12 text-emerald-600 mb-4" />
              <h3 className="text-xl font-semibold text-slate-900 mb-4">99.99% Uptime</h3>
              <p className="text-slate-600">Guaranteed availability with redundant infrastructure and 24/7 monitoring across global data centers.</p>
            </div>
          </div>
        </div>
      </section>

      {/* FAQ Section */}
      <section className="py-20 px-4 sm:px-6 lg:px-8" data-landright-section="FAQ">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            <h2 className={`${playfair.className} text-4xl md:text-5xl font-bold text-slate-900 mb-6`}>
              Frequently Asked Questions
            </h2>
            <p className="text-xl text-slate-600">
              Everything you need to know about building your digital empire with Whop.
            </p>
          </div>
          
          <div className="space-y-6">
            {[
              {
                q: 'What fees does Whop charge for transactions?',
                a: 'Whop charges a competitive 2.9% + $0.30 per transaction, with volume discounts available for high-volume merchants. No monthly fees or hidden charges.'
              },
              {
                q: 'How quickly can I set up my digital storefront?',
                a: 'Most merchants are live and selling within 15 minutes. Our streamlined onboarding process and pre-built templates make setup incredibly fast while maintaining professional quality.'
              },
              {
                q: 'What types of digital products can I sell?',
                a: 'Sell any digital product including courses, ebooks, software, memberships, subscriptions, NFTs, digital art, templates, and more. Our platform handles all file types and delivery methods.'
              },
              {
                q: 'Which payment methods do you support?',
                a: 'We support all major credit cards, PayPal, Apple Pay, Google Pay, bank transfers, and cryptocurrency payments. Global payment processing in 135+ currencies.'
              },
              {
                q: 'What kind of customer support do you provide?',
                a: '24/7 premium support via live chat, email, and phone. Dedicated account managers for enterprise clients plus comprehensive documentation and video tutorials.'
              },
              {
                q: 'How does Whop help me scale my business?',
                a: 'Advanced analytics, automated marketing tools, affiliate program management, API integrations, and white-label solutions. Plus access to our marketplace for additional discovery.'
              }
            ].map((faq, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: index * 0.1 }}
                className="bg-white rounded-2xl p-8 shadow-lg border border-slate-200"
              >
                <h3 className="text-xl font-semibold text-slate-900 mb-4">{faq.q}</h3>
                <p className="text-slate-600 leading-relaxed">{faq.a}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Final CTA Section */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-violet-900 via-purple-900 to-slate-900" data-landright-section="FinalCTA">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className={`${playfair.className} text-4xl md:text-6xl font-bold text-white mb-6`}>
            Ready to Build Your Empire?
          </h2>
          <p className="text-xl text-slate-300 mb-12 max-w-3xl mx-auto">
            Join thousands of successful merchants who've chosen Whop as their digital commerce platform. 
            Start your journey to seven-figure success today.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
            <a href="https://www.linkedin.com/company/whop-io/about/" className="bg-gradient-to-r from-amber-500 to-yellow-600 text-slate-900 px-8 py-4 rounded-xl font-bold text-lg hover:from-amber-400 hover:to-yellow-500 transition-all duration-200 shadow-xl hover:shadow-2xl hover:scale-105 inline-flex items-center gap-2">
              Get started
              <ArrowRight className="w-5 h-5" />
            </a>
            <div className="flex items-center gap-2 text-slate-300">
              <Headphones className="w-5 h-5" />
              <span>24/7 Premium Support Included</span>
            </div>
          </div>
        </div>
      </section>

      {/* Footer */}
      <footer className="bg-slate-900 text-white py-16 px-4 sm:px-6 lg:px-8" data-landright-section="Footer">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
            <div>
              <div className="flex items-center space-x-3 mb-6">
                <img src="data:image/webp;base64," alt="Whop" className="h-8 w-auto" />
                <span className="text-xl font-semibold">Whop</span>
              </div>
              <p className="text-slate-400 mb-6">
                Your digital storefront for the new economy. Empowering creators and entrepreneurs worldwide.
              </p>
            </div>
            
            <div>
              <h4 className="font-semibold mb-4">Platform</h4>
              <ul className="space-y-2 text-slate-400">
                <li>Storefront Builder</li>
                <li>Payment Processing</li>
                <li>Analytics Dashboard</li>
                <li>Mobile Apps</li>
              </ul>
            </div>
            
            <div>
              <h4 className="font-semibold mb-4">Resources</h4>
              <ul className="space-y-2 text-slate-400">
                <li>Documentation</li>
                <li>API Reference</li>
                <li>Success Stories</li>
                <li>Community</li>
              </ul>
            </div>
            
            <div>
              <h4 className="font-semibold mb-4">Support</h4>
              <ul className="space-y-2 text-slate-400">
                <li>Help Center</li>
                <li>Contact Support</li>
                <li>System Status</li>
                <li>Security</li>
              </ul>
            </div>
          </div>
          
          <div className="border-t border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center">
            <p className="text-slate-400 mb-4 md:mb-0">
              © 2024 Whop. All rights reserved.
            </p>
            <div className="flex space-x-6 text-slate-400">
              <span className="hover:text-white cursor-pointer transition-colors">Privacy Policy</span>
              <span className="hover:text-white cursor-pointer transition-colors">Terms of Service</span>
              <span className="hover:text-white cursor-pointer transition-colors">Cookie Policy</span>
            </div>
          </div>
        </div>
      </footer>
    </div>
  );
}
