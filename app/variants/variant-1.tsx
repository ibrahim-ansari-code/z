"use client";

import {Bebas_Neue, Manrope} from 'next/font/google';
import { motion } from 'framer-motion';
import { ArrowRight, Shield, Zap, TrendingUp, Users, CreditCard, Smartphone, Eye, Star, CheckCircle } from 'lucide-react';

const bebasNeue = Bebas_Neue({ subsets: ['latin'], weight: '400' });
const manrope = Manrope({ subsets: ['latin'] });

export default function WhopLanding() {
  const floatingCards = [
    { title: 'Digital Courses', price: '$2,499', sales: '847 sold', x: 20, y: 10 },
    { title: 'Premium Tools', price: '$99', sales: '1.2k sold', x: 70, y: 20 },
    { title: 'Exclusive Content', price: '$49', sales: '3.4k sold', x: 15, y: 60 },
    { title: 'Software Access', price: '$199', sales: '567 sold', x: 75, y: 70 }
  ];

  return (
    <div className={`min-h-screen bg-gray-900 text-white ${manrope.className}`}>
      {/* Navigation */}
      <nav className="fixed top-0 w-full bg-gray-900/95 backdrop-blur-sm border-b border-gray-800 z-50" data-landright-section="Navigation">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              <img src="data:image/webp;base64," alt="Whop" className="h-8 w-auto" />
              <span className={`text-xl font-bold text-purple-400 ${bebasNeue.className}`}>WHOP</span>
            </div>
            <div className="hidden md:flex items-center space-x-8">
              <span className="text-gray-300 hover:text-white transition-colors cursor-pointer">Marketplace</span>
              <span className="text-gray-300 hover:text-white transition-colors cursor-pointer">Sellers</span>
              <span className="text-gray-300 hover:text-white transition-colors cursor-pointer">Support</span>
            </div>
          </div>
        </div>
      </nav>

      {/* Hero Section */}
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16" data-landright-section="Hero">
        <div className="absolute inset-0 bg-gradient-to-br from-purple-900/20 via-gray-900 to-orange-900/20" />
        <div className="absolute inset-0">
          {floatingCards.map((card, index) => (
            <motion.div
              key={index}
              className="absolute bg-gray-800/80 backdrop-blur-sm border border-gray-700 rounded-lg p-4 shadow-lg"
              style={{ left: `${card.x}%`, top: `${card.y}%` }}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.2, duration: 0.6 }}
              whileHover={{ scale: 1.05, y: -5 }}
            >
              <div className="text-sm font-semibold text-white">{card.title}</div>
              <div className="text-purple-400 font-bold">{card.price}</div>
              <div className="text-xs text-gray-400">{card.sales}</div>
            </motion.div>
          ))}
        </div>
        
        <div className="relative z-10 text-center max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.h1 
            className={`text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold mb-6 ${bebasNeue.className}`}
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
          >
            <span className="text-white">BUILD YOUR</span><br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-orange-400">DIGITAL EMPIRE</span>
          </motion.h1>
          
          <motion.p 
            className="text-xl sm:text-2xl text-gray-300 mb-8 max-w-2xl mx-auto leading-relaxed"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3, duration: 0.8 }}
          >
            Your digital storefront for the new economy. Join thousands of creators building profitable businesses with our all-in-one platform.
          </motion.p>
          
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.6, duration: 0.8 }}
          >
            <a 
              href="https://www.linkedin.com/company/whop-io/about/"
              className="inline-flex items-center px-8 py-4 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-purple-500/25"
            >
              Get started
              <ArrowRight className="ml-2 h-5 w-5" />
            </a>
          </motion.div>
        </div>
      </section>

      {/* Social Proof Section */}
      <section className="py-16 bg-gray-800/50" data-landright-section="SocialProof">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-2">
              <div className={`text-4xl sm:text-5xl font-bold text-purple-400 ${bebasNeue.className}`}>$1B+</div>
              <div className="text-gray-300">Payments Processed</div>
            </div>
            <div className="space-y-2">
              <div className={`text-4xl sm:text-5xl font-bold text-orange-400 ${bebasNeue.className}`}>10K+</div>
              <div className="text-gray-300">Active Merchants</div>
            </div>
            <div className="space-y-2">
              <div className={`text-4xl sm:text-5xl font-bold text-purple-400 ${bebasNeue.className}`}>95%</div>
              <div className="text-gray-300">Seller Satisfaction</div>
            </div>
          </div>
        </div>
      </section>

      {/* Features Section */}
      <section className="py-20 bg-gray-900" data-landright-section="Features">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className={`text-4xl sm:text-5xl font-bold mb-6 text-white ${bebasNeue.className}`}>
              EVERYTHING YOU NEED TO SELL
            </h2>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              From storefront creation to payment processing, we handle the technical complexity so you can focus on growing your business.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: Zap, title: 'Storefront Builder', desc: 'Create stunning digital storefronts in minutes with our drag-and-drop builder' },
              { icon: CreditCard, title: 'Payment Processing', desc: 'Secure, instant payments with support for all major payment methods globally' },
              { icon: TrendingUp, title: 'Marketplace Discovery', desc: 'Get discovered by millions of buyers in our curated marketplace ecosystem' },
              { icon: Eye, title: 'Analytics Dashboard', desc: 'Track sales, customer behavior, and optimize your business with detailed insights' },
              { icon: Users, title: 'Customer Management', desc: 'Build relationships with integrated CRM tools and automated communication' },
              { icon: Shield, title: 'Digital Delivery', desc: 'Automated, secure delivery of digital products with anti-piracy protection' },
              { icon: Smartphone, title: 'Mobile Optimization', desc: 'Your storefront looks perfect on every device with responsive design' },
              { icon: CheckCircle, title: 'Fraud Protection', desc: 'Advanced security measures protect both you and your customers' }
            ].map((feature, index) => (
              <motion.div
                key={index}
                className="bg-gray-800/50 border border-gray-700 rounded-lg p-6 hover:border-purple-500/50 transition-all duration-300"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.1, duration: 0.6 }}
                viewport={{ once: true }}
              >
                <feature.icon className="h-8 w-8 text-purple-400 mb-4" />
                <h3 className="text-lg font-semibold text-white mb-2">{feature.title}</h3>
                <p className="text-gray-400 text-sm">{feature.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* How It Works Section */}
      <section className="py-20 bg-gray-800/30" data-landright-section="Process">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className={`text-4xl sm:text-5xl font-bold mb-6 text-white ${bebasNeue.className}`}>
              START SELLING IN 4 STEPS
            </h2>
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">
              Launch your digital business faster than ever with our streamlined onboarding process.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { step: '01', title: 'Sign Up', desc: 'Create your account in under 2 minutes with just your email' },
              { step: '02', title: 'Create Storefront', desc: 'Design your brand with our intuitive storefront builder' },
              { step: '03', title: 'Upload Products', desc: 'Add your digital products with automated delivery setup' },
              { step: '04', title: 'Start Selling', desc: 'Go live instantly and start accepting payments worldwide' }
            ].map((item, index) => (
              <motion.div
                key={index}
                className="text-center"
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.2, duration: 0.6 }}
                viewport={{ once: true }}
              >
                <div className={`text-6xl font-bold text-purple-400 mb-4 ${bebasNeue.className}`}>{item.step}</div>
                <h3 className="text-xl font-semibold text-white mb-3">{item.title}</h3>
                <p className="text-gray-400">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Testimonials Section */}
      <section className="py-20 bg-gray-900" data-landright-section="Testimonials">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className={`text-4xl sm:text-5xl font-bold mb-6 text-white ${bebasNeue.className}`}>
              SUCCESS STORIES
            </h2>
            <p className="text-xl text-gray-300 max-w-2xl mx-auto">
              Join thousands of creators who have transformed their passion into profitable businesses.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: 'Sarah Chen', business: 'Digital Marketing Courses', revenue: '$127K', quote: 'Whop transformed my side hustle into a six-figure business. The platform handles everything so I can focus on creating content.' },
              { name: 'Marcus Rodriguez', business: 'Trading Signals', revenue: '$89K', quote: 'The automated delivery and customer management features saved me 20 hours per week. My subscribers love the seamless experience.' },
              { name: 'Emma Thompson', business: 'Design Templates', revenue: '$156K', quote: 'I went from struggling freelancer to running a profitable template business. The marketplace exposure brought me customers I never could have reached alone.' }
            ].map((testimonial, index) => (
              <motion.div
                key={index}
                className="bg-gray-800/50 border border-gray-700 rounded-lg p-6"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.2, duration: 0.6 }}
                viewport={{ once: true }}
              >
                <div className="flex items-center mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="h-4 w-4 text-orange-400 fill-current" />
                  ))}
                </div>
                <p className="text-gray-300 mb-6 italic">"{testimonial.quote}"</p>
                <div className="border-t border-gray-700 pt-4">
                  <div className="font-semibold text-white">{testimonial.name}</div>
                  <div className="text-sm text-gray-400">{testimonial.business}</div>
                  <div className={`text-lg font-bold text-purple-400 mt-1 ${bebasNeue.className}`}>{testimonial.revenue} Revenue</div>
                </div>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* FAQ Section */}
      <section className="py-20 bg-gray-800/30" data-landright-section="FAQ">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className={`text-4xl sm:text-5xl font-bold mb-6 text-white ${bebasNeue.className}`}>
              FREQUENTLY ASKED QUESTIONS
            </h2>
          </div>
          
          <div className="space-y-6">
            {[
              { q: 'What fees does Whop charge?', a: 'We charge a simple 5% transaction fee on successful sales. No monthly fees, no setup costs, no hidden charges. You only pay when you make money.' },
              { q: 'How quickly can I set up my storefront?', a: 'Most sellers have their storefront live within 30 minutes. Our guided setup process walks you through everything from branding to product uploads.' },
              { q: 'What types of digital products can I sell?', a: 'Courses, ebooks, software, templates, memberships, coaching sessions, digital art, music, and more. If it\'s digital, you can sell it on Whop.' },
              { q: 'Which payment methods do you support?', a: 'We support all major credit cards, PayPal, Apple Pay, Google Pay, and cryptocurrency payments. Your customers can pay however they prefer.' },
              { q: 'What kind of support do you provide?', a: '24/7 customer support via chat and email, comprehensive documentation, video tutorials, and a community forum of successful sellers.' },
              { q: 'Can I scale my business as I grow?', a: 'Absolutely. Our platform handles everything from your first sale to millions in revenue. Advanced analytics, automation tools, and enterprise features scale with your success.' }
            ].map((faq, index) => (
              <motion.div
                key={index}
                className="bg-gray-800/50 border border-gray-700 rounded-lg p-6"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.1, duration: 0.6 }}
                viewport={{ once: true }}
              >
                <h3 className="text-lg font-semibold text-white mb-3">{faq.q}</h3>
                <p className="text-gray-300">{faq.a}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Trust Indicators Section */}
      <section className="py-20 bg-gray-900" data-landright-section="Trust">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className={`text-3xl sm:text-4xl font-bold mb-12 text-white ${bebasNeue.className}`}>
            TRUSTED & SECURE
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-4">
              <Shield className="h-12 w-12 text-purple-400 mx-auto" />
              <h3 className="text-xl font-semibold text-white">Bank-Level Security</h3>
              <p className="text-gray-400">SSL encryption, PCI compliance, and fraud protection keep your business and customers safe.</p>
            </div>
            <div className="space-y-4">
              <CheckCircle className="h-12 w-12 text-orange-400 mx-auto" />
              <h3 className="text-xl font-semibold text-white">99.9% Uptime</h3>
              <p className="text-gray-400">Enterprise-grade infrastructure ensures your storefront is always available to customers.</p>
            </div>
            <div className="space-y-4">
              <Users className="h-12 w-12 text-purple-400 mx-auto" />
              <h3 className="text-xl font-semibold text-white">Proven Track Record</h3>
              <p className="text-gray-400">Trusted by over 10,000 sellers who have collectively earned millions through our platform.</p>
            </div>
          </div>
        </div>
      </section>

      {/* Footer */}
      <footer className="bg-gray-900 border-t border-gray-800 py-12" data-landright-section="Footer">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="flex items-center space-x-3 mb-6 md:mb-0">
              <img src="data:image/webp;base64," alt="Whop" className="h-8 w-auto" />
              <span className={`text-xl font-bold text-purple-400 ${bebasNeue.className}`}>WHOP</span>
            </div>
            
            <div className="text-center md:text-right">
              <p className="text-gray-400 text-sm">
                © 2024 Whop. Your digital storefront for the new economy.
              </p>
            </div>
          </div>
        </div>
      </footer>
    </div>
  );
}
